...
int main (void)
{
	...
	// Output
        ...
        fprintf(summary,"mean\tshape\tN\tN\tdom_1\tdom_2\tcoexist\textinct\tff\n");
    ...
    for (jj=0;jj<env_shape_num;jj++){
        ...
        sd_tmp_long=    short_term[jj];
        sd_tmp_short=   long_term[jj];
        //
        count_coe= count_spe= count_gen= count_ext= count_fal= count_sim= 0;
    ...
    // print result 
        fprintf(summary,"%lf\t%lf\t%d\t%d\t%d\t%d\t%d\t%d\t%d\n",mean_tmp,sd_tmp_long,num_sim,count_sim,count_spe,count_gen,count_coe,count_ext,count_fal);
